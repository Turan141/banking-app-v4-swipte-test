...
// Req's
export const REQ_TRANSACTIONS_HISTORY: Req<string, ITransactionOperation[]> =
  new Req();
...
class AccountHomeManager {
  ...
  async init() {
    // Req's
    REQ_TRANSACTIONS_HISTORY.listener = async () =>
      await this.getTransactionsOperationHistory();
  ...
  async getTransactionDetails(): Promise<TransactionsHistoryItem> {
    console.log(this.transactionOpenedUID);
    // this.transactionOpenedUID is uid of transaction that was opened
  ...
  async getTransactionsOperationHistory(): Promise<any> {
    // todo GET money/history
    const groupedOperationsByDate = this.groupTransactionsByDate(
      transactionOperationHistory.data as ITransactionOperation[]
    );
